...
from utils.debug_functions import debug_model_dimensions
from utils.extended_metrics import compute_extended_metrics, flatten_for_wandb, save_metrics_json, compute_efficiency
from tensorboard import TensorboardLogger
from models.mambavision_fcm_net import *
...
try:
    if getattr(torch.amp, 'autocast', None) is not None:
        has_native_amp = True
...
# ✅ Add these new arguments for ablation studies
parser.add_argument('--patch_embed', type=str, default='default', help="Patch embedding type")
parser.add_argument('--first_layer', type=str, default='default', help="First stage block type")
parser.add_argument('--second_layer', type=str, default='default', help="Second stage block type")
parser.add_argument('--mixer', type=str, default='default', help="Mixer block type")
parser.add_argument('--group', type=str, default='default_group', help="WandB group name")
parser.add_argument('--color_jitter', type=str, default='default_group', help="Color jitter percentage")
parser.add_argument('--fcm_dir', type=str, default='', help="Directory of precomputed FCM PNG maps. If set, enables 4-channel FCM fusion input.")
...
def _parse_args():
    ...
    if args_config.config:
        with open(args_config.config, 'r') as f:
            ...
            # Set all config values as defaults for argparse
...
def main():
    ...
    if args.log_wandb:
        if has_wandb:
            wandb.init(project=args.group, 
                       group=args.model, 
                       name=args.experiment, config=args)
        else:
            _logger.warning("You've requested to log metrics to wandb but package not found. "
                            "Metrics not being logged to wandb, try `pip install wandb`")

    args.prefetcher = not args.no_prefetcher
    # FCM path uses a plain DataLoader (no timm prefetcher), so disable prefetcher
    # to ensure input/target are moved to GPU inside train_one_epoch.
    if hasattr(args, 'fcm_dir') and args.fcm_dir:
    ...
    if 'T2_baseline' in args.model:
        model = create_model(
            ...
            # mixer=args.mixer if  hasattr(args, "mixer") else None
        )
    elif 'mambavision_fcm_net' == args.model:
        model = create_model(
            args.model,
            pretrained=args.pretrained,
            num_classes=args.num_classes,
            in_chans=getattr(args, 'in_chans', 4) if (hasattr(args, 'fcm_dir') and args.fcm_dir) else 3,
            global_pool=args.gp,
            bn_momentum=args.bn_momentum,
            bn_eps=args.bn_eps,
            scriptable=args.torchscript,
    ...
    elif 'mamba_vision_T3_pyramid_fusion' == args.model or 'mamba_vision_T4_pyramid_fusion_refinement' == args.model:
        model = create_model(
            args.model,
            pretrained=args.pretrained,
            num_classes=args.num_classes,
    ...
    elif 'mamba_vision_T5_refinement' == args.model or 'mamba_vision_T6_refinement_attn' == args.model:
        model = create_model(
            args.model,
            pretrained=args.pretrained,
            num_classes=args.num_classes,
    ...
    if args.loadcheckpoint:
        ...
        raw = torch.load(args.loadcheckpoint, weights_only=False)
        new_model_weights = raw.get('state_dict', raw.get('model', raw))
        current_model = model.state_dict()

        # Inflate 3-channel stem to 4-channel for FCM fusion
        stem_key = 'patch_embed.conv_down.0.weight'
        if stem_key in new_model_weights and stem_key in current_model:
            ckpt_w = new_model_weights[stem_key]   # (out, 3, kH, kW)
            model_w = current_model[stem_key]       # (out, 4, kH, kW)
            if ckpt_w.shape[1] != model_w.shape[1]:
                extra = ckpt_w.mean(dim=1, keepdim=True).expand(
                    ckpt_w.shape[0], model_w.shape[1] - ckpt_w.shape[1], *ckpt_w.shape[2:])
                new_model_weights[stem_key] = torch.cat([ckpt_w, extra], dim=1)
                print(f"Inflated {stem_key}: {ckpt_w.shape[1]}ch -> {model_w.shape[1]}ch")

        new_state_dict = OrderedDict()
        for k in current_model.keys():
            if k in new_model_weights and new_model_weights[k].size() == current_model[k].size():
                new_state_dict[k] = new_model_weights[k]

        model.load_state_dict(new_state_dict, strict=False)
        print(f"Loaded {len(new_state_dict)}/{len(current_model)} params from {args.loadcheckpoint}")
        # model_ema.module.load_state_dict(new_state_dict, strict=False)

    # ── Compute model efficiency metrics once ──────────────────────────────────
    in_chans_eff = 4 if (hasattr(args, 'fcm_dir') and args.fcm_dir) else 3
    params_M, flops_G, inference_ms = compute_efficiency(model, in_chans=in_chans_eff)
    print(f"Model efficiency — Params: {params_M}M | FLOPs: {flops_G}G | Inference: {inference_ms}ms")
    if args.log_wandb and has_wandb and wandb.run is not None:
        wandb.run.summary.update({'params_M': params_M, 'flops_G': flops_G, 'inference_ms': inference_ms})
    ...
    # Use FCM-aware loader if fcm_dir is set
    if hasattr(args, 'fcm_dir') and args.fcm_dir:
        from utils.imagefolder_with_fcm import ImageFolderWithFCM
        import torchvision.transforms as T
        train_dir = os.path.join(args.data_dir, 'train')
        val_dir = os.path.join(args.data_dir, 'validation')
        # Build standard timm-style transforms for image channels
        normalize = T.Normalize(mean=data_config['mean'], std=data_config['std'])
        train_tf = T.Compose([
            T.RandomResizedCrop(data_config['input_size'][-1], scale=args.scale, ratio=args.ratio),
            T.RandomHorizontalFlip(p=args.hflip),
            T.ColorJitter(brightness=float(args.color_jitter) if args.color_jitter not in ('', 'default_group') else 0.4,
                          contrast=float(args.color_jitter) if args.color_jitter not in ('', 'default_group') else 0.4,
                          saturation=float(args.color_jitter) if args.color_jitter not in ('', 'default_group') else 0.4),
            T.ToTensor(),
            normalize,
        ])
        val_tf = T.Compose([
            T.Resize(int(data_config['input_size'][-1] / data_config['crop_pct'])),
            T.CenterCrop(data_config['input_size'][-1]),
            T.ToTensor(),
            normalize,
        ])
        dataset_train = ImageFolderWithFCM(train_dir, args.fcm_dir, transform=train_tf)
        dataset_eval = ImageFolderWithFCM(val_dir, args.fcm_dir, transform=val_tf)
        # Build plain DataLoaders — bypass timm's create_loader/prefetcher which
        # expects raw PIL images and would conflict with our pre-tensorized 4-ch output.
        ...
        print(f"\nDataset sizes: TRAIN - {len(dataset_train)} and EVAL - {len(dataset_eval)}\n")
        # Skip the rest of the data setup and jump straight to training
        collate_fn = None
        mixup_fn = None
    ...
    if not (hasattr(args, 'fcm_dir') and args.fcm_dir):
    ...
    # Apply class weights for BUSI imbalance: benign=437, malignant=210, normal=133
    # w_c = N / (C * n_c) -> [0.595, 1.238, 1.955]
    if hasattr(args, 'fcm_dir') and args.fcm_dir and args.num_classes == 3:
        class_weights = torch.tensor([0.595, 1.238, 1.955]).cuda()
        if isinstance(train_loss_fn, nn.CrossEntropyLoss):
            train_loss_fn = nn.CrossEntropyLoss(weight=class_weights)
        elif isinstance(train_loss_fn, LabelSmoothingCrossEntropy):
            # Wrap with weighted CE since timm's LabelSmoothingCrossEntropy doesn't support weights
            train_loss_fn = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=args.smoothing)
    ...
    if args.rank == 0:
        ...
        with open(os.path.join(output_dir, 'model.txt'), 'w') as f:
            f.write(str(model))
            if args.log_wandb and has_wandb and wandb.run is not None:
                artifact = wandb.Artifact(name="model-structure", type="text")
                artifact.add_file(os.path.join(output_dir, 'model.txt'))  # could be .csv, .json, .pth, etc.
                wandb.log_artifact(artifact)
        
        with open(os.path.join(output_dir, f'model_statistics.txt'), 'w') as f:
            in_chans = 4 if (hasattr(args, 'fcm_dir') and args.fcm_dir) else 3
            f.write(str(summary(model, input_size=(1, in_chans, 224, 224)))) 
            

        if 1: #args.copy_code
            # copy .py files, excluding venv and output dirs
            files = [os.path.join(dp, f) for dp, dn, filenames in os.walk('.')
                     for f in filenames
                     if os.path.splitext(f)[1] == '.py'
                     and '/venv/' not in os.path.join(dp, f)
                     and '/code_copy/' not in os.path.join(dp, f)]
            for f in files:
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                # Use EMA metrics only if they are better than the live model
                if ema_eval_metrics['top1'] > eval_metrics['top1']:
                    # carry over raw arrays from whichever is better
                    eval_metrics = ema_eval_metrics

            # ── Extended metrics ────────────────────────────────────────────────
            preds   = eval_metrics.pop('_preds',   None)
            targets = eval_metrics.pop('_targets', None)
            probs   = eval_metrics.pop('_probs',   None)

            ext = {}
            if preds is not None and output_dir is not None:
                import numpy as np
                class_names = getattr(loader_eval.dataset, 'classes', None)
                ext = compute_extended_metrics(targets, preds, probs,
                                               class_names=class_names, n_boot=500)
                # Merge top-level scalars into eval_metrics for timm's update_summary
                eval_metrics['precision'] = ext['precision_macro']
                eval_metrics['recall']    = ext['recall_macro']
                eval_metrics['f1']        = ext['f1_macro']

                # Write per-epoch JSON
                json_path = os.path.join(output_dir, f'metrics_epoch_{epoch:04d}.json')
                full_record = {
                    'epoch': epoch,
                ...
                save_metrics_json(full_record, json_path)

                # Also write/overwrite a best_metrics.json when this is the best epoch
                if eval_metrics['top1'] >= best_top1:
                    save_metrics_json(full_record, os.path.join(output_dir, 'best_metrics.json'))
            ...
            if args.log_wandb and has_wandb and wandb.run is not None:
                ...
                if ext:
                    wandb_payload.update(flatten_for_wandb(ext, prefix='val'))
                wandb.log(wandb_payload)

            curr_acc_max = eval_metrics['top1']
            if curr_acc_max > max_acc:
                max_acc = curr_acc_max
            if curr_acc_max > best_top1:
                best_top1 = curr_acc_max
                epochs_without_improvement = 0
            else:
                epochs_without_improvement += 1
            if args.log_wandb and has_wandb and wandb.run is not None:
                wandb.run.summary['max_acc'] = max_acc
            ...
            should_early_stop = args.patience_epochs > 0 and epochs_without_improvement >= args.patience_epochs

            if not np.isfinite(eval_metrics['loss']):
                if args.local_rank == 0:
                    _logger.info("Nan in loss, stopping training")
                    _logger.error("Nan in loss, stopping training")
                break

            if should_early_stop:
                if args.local_rank == 0:
                    _logger.info(
                        "Early stopping triggered: no top1 improvement for %d epochs (patience=%d).",
    ...
    if best_metric is not None:
        ...
        with open(os.path.join(output_dir, 'best.txt'), 'w') as f:
            f.write(f"Experiment {args.experiment} - best_top1={best_metric} at step={best_epoch}") 

        if args.log_wandb and has_wandb:
            print(f"[wandb] Logging best_top1={best_metric} at step={best_epoch}")
