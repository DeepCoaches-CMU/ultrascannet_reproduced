...
class MobileViTBlock(nn.Module):
    ...
    def __init__(self, dim, drop_path=0.0, layer_scale=None, num_layers=2, use_spatial_gating=True):
        super().__init__()
        # Local representation: depthwise + pointwise conv
        ...
        # Efficient transformer: use lightweight attention (linear attention)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=dim, nhead=4, dim_feedforward=dim * 2, batch_first=True, norm_first=True, activation='gelu')
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.norm2 = nn.LayerNorm(dim)
        # Spatial Gating Unit (SGU) v2: depthwise conv + sigmoid
        self.use_spatial_gating = use_spatial_gating
        self.sgu_gate = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim),
            nn.Sigmoid()
        )
        # Fusion: residual + pointwise conv
    ...
    def forward(self, x):
        ...
        x_patch = self.norm1(x_patch)
        # Global features (efficient transformer)
        x_patch = self.transformer(x_patch)
        x_patch = self.norm2(x_patch)
        x_patch = x_patch.transpose(1, 2).reshape(B, C, H, W)
        # Spatial gating
        if self.use_spatial_gating:
            gate = self.sgu_gate(x_patch)
            x_patch = x_patch * gate
        # Concatenate local and global, then fuse
        x_fused = torch.cat([x_local, x_patch], dim=1)  # [B, 2C, H, W]
        out = self.fuse(x_fused)
